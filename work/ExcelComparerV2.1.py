...
class CompareWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 检查规则中的列是否在源文件和目标文件中都存在
            columns_to_compare = list(self.rules.keys())

            missing_in_file1 = [col for col in columns_to_compare if col not in df1.columns]
            missing_in_file2 = [col for col in columns_to_compare if col not in df2.columns]

            if missing_in_file1 or missing_in_file2:
                error_msg = ""
                if missing_in_file1:
                    error_msg += f"源文件缺失以下规则定义的列：{', '.join(missing_in_file1)}\n"
                if missing_in_file2:
                    error_msg += f"目标文件缺失以下规则定义的列：{', '.join(missing_in_file2)}\n"
            ...
            if not missing_in_file1.empty:
                ...
                self.log_signal.emit(f"【目标文件中多出的主键】（源文件中没有，共 {len(missing_in_file1)} 条）：\n{missing_list_file1}")
            ...
            for code in common_codes:
                ...
                source_row = df1_common.loc[code_str]
                target_row = df2_common.loc[code_str]

                for field1, rule in self.rules.items():
                    # 只比对规则文件中定义的列
                    if field1 not in df1.columns or field1 not in df2.columns:
                        continue

                    data_type = rule["data_type"]
                    tail_diff = rule.get("tail_diff")  # 不设置默认值，保留 None

                    val1 = source_row.get(field1, '')
                    val2 = target_row.get(field1, '')  # 已经映射为 field1
                    ...
                    if data_type == "数值":
                            ...
                            if tail_diff is None:
                                if val1_num != val2_num:
                                    if code not in diff_dict:
                                        diff_dict[code] = []
                                    diff_dict[code].append((field1, val1, val2))
                            else:
                        ...
                        try:
                            if CompareWorker.normalize_value(val1) == '' and CompareWorker.normalize_value(
                                    val2) == '':
                                continue
                            ...
                            if tail_diff == "月":
                                # 只比较年月部分，格式如 2010-10
                                val1_cmp = val1[:7] if len(val1) >= 7 else val1
                                val2_cmp = val2[:7] if len(val2) >= 7 else val2
                            elif tail_diff == "日":
                                # 只比较年月日部分，格式如 2010-10-06
                                val1_cmp = val1[:10] if len(val1) >= 10 else val1
                                val2_cmp = val2[:10] if len(val2) >= 10 else val2
                            elif tail_diff == "时":
                                # 只比较到小时，格式如 2010-10-06 15
                                val1_cmp = val1[:13] if len(val1) >= 13 else val1
                                val2_cmp = val2[:13] if len(val2) >= 13 else val2
                            elif tail_diff == "分":
                                # 只比较到分钟，格式如 2010-10-06 15:30
                                val1_cmp = val1[:16] if len(val1) >= 16 else val1
                                val2_cmp = val2[:16] if len(val2) >= 16 else val2
                            elif tail_diff == "秒":
                                # 只比较到秒，格式如 2010-10-06 15:30:45
                                val1_cmp = val1[:19] if len(val1) >= 19 else val1
                                val2_cmp = val2[:19] if len(val2) >= 19 else val2
                            else:
                                # 默认只比较年份
                                val1_cmp = val1[:4] if len(val1) >= 4 else val1
                                val2_cmp = val2[:4] if len(val2) >= 4 else val2
...
class ExcelComparer(QWidget):
    ...
    def __init__(self):
        ...
        rule_file_path = os.path.join(current_dir, "rule.xlsx")
        if os.path.exists(rule_file_path):
            ...
            self.log("未找到规则文件 rule.xlsx")
            return

    def initUI(self):
        """初始化用户界面"""
        self.setWindowTitle("Excel文件比较工具V2.1")
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, 'worker') and self.worker.isRunning():
            self.worker.quit()
            self.worker.wait()
        if hasattr(self, 'worker_load1') and self.worker_load1.isRunning():
            self.worker_load1.quit()
            self.worker_load1.wait()
        if hasattr(self, 'worker_load2') and self.worker_load2.isRunning():
        ...
        if hasattr(self, 'worker_sheet2') and self.worker_sheet2.isRunning():
    ...
    def compare_files(self):
        ...
        self.worker = CompareWorker(self.file1, self.file2, sheet_name1, sheet_name2, primary_keys=primary_keys, rules=self.rules)
