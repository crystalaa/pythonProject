...
class CompareWorker(QThread):
    ...
    def run(self):
        ...
        if missing_columns:
            self.log_signal.emit(f"错误：目标文件缺少以下列：{', '.join(missing_columns)}，请检查文件结构！")
            return
        ...
        if not missing_in_file2.empty:
            ...
            # original_codes = missing_in_file2.astype(str)
            # missing_df.reset_index(drop=True, inplace=True)
        ...
        # 按照列名对 df2_compare 进行重排序，确保列顺序与 df1_compare 一致
        df2_compare_aligned = df2_compare[df1_compare.columns]

        # 现在可以安全地进行逐行比较
        mask = (df1_compare == df2_compare_aligned).all(axis=1)
        diff_rows = df1_compare[~mask]
        ...
        for code, row in diff_rows.iterrows():
            ...
            for col in df1_compare.columns:
                val1 = df1_compare.at[code, col]
                val2 = df2_compare.at[code, col]
                val1 = self.normalize_value(val1)
                val2 = self.normalize_value(val2)

                if val1 != val2:
                # if (isinstance(val1, pd.Series) and not val1.empty) or (isinstance(val2, pd.Series) and not val2.empty):
                #     # 如果是 Series，取第一个值
                #     val1 = val1.iloc[0] if isinstance(val1, pd.Series) else val1
                #     val2 = val2.iloc[0] if isinstance(val2, pd.Series) else val2
                # if (val1 or val2) and str(val1) != str(val2):  # 强制转换为字符串比较
        ...
        self.summary = {
            ...
            "extra_count": len(missing_in_file1),  # 新增字段
...
class ExcelComparer(QWidget):
    ...
    def on_columns_loaded(self, file_path, columns):
        ...
        if source_cols.issubset(target_cols) :
