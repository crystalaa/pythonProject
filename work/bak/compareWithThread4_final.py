...
class CompareWorker(QThread):
    ...
    def run(self):
        ...
        if missing_columns:
            self.log_signal.emit(f"错误：目标文件缺少以下列：{', '.join(missing_columns)}，将视为空值处理。")
        ...
        df1_compare.index = df1_compare.index.map(lambda x: ' + '.join(x) if isinstance(x, tuple) else str(x))
        df2_compare.index = df2_compare.index.map(lambda x: ' + '.join(x) if isinstance(x, tuple) else str(x))
        # 计算差异
        diff_count = 0
        diff_log_messages = []
        ...
        for code, row in diff_rows.iterrows():
            ...
            for col in df1_compare.columns:
                ...
                val2 = df2_compare.at[code_str, col]
...
class ExcelComparer(QWidget):
    ...
    def on_columns_loaded(self, file_path, columns):
        ...
        if source_cols.issubset(target_cols) :
            ...
            self.log("两个文件的列名不一致，使用源文件字段进行对比。")
            self.update_primary_key_list(self.columns1)
