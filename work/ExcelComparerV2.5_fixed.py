...
def read_rules(file_path):
    """读取规则文件，返回规则字典（新增计算规则解析）"""
    try:
        wb = load_workbook(filename=file_path, read_only=True, data_only=True)
        ws = wb['比对规则']
        rules = {}

        for row in ws.iter_rows(min_row=2, values_only=True):  # 第一行是标题
            # 解包时增加calc_rule（假设第6列是计算规则）
    ...
    def run(self):
        try:
            ...
            for field1, rule in self.rules.items():
                ...
                table2_field = rule["table2_field"]
                # 只要有计算规则，就执行计算并覆盖表二字段（无论该字段是否已存在）
                if calc_rule:
                    self.log_signal.emit(f"根据规则计算表二字段：{table2_field} = {calc_rule}")
                    # 执行计算并覆盖字段值
                    df2[table2_field] = self.calculate_field(df2, calc_rule, rule["data_type"])
                    # 检查计算结果是否有效
                    if df2[table2_field].isna().all():
            ...
            # 确保所有需要的列都在df1和df2中存在
            for col in all_needed_columns:
                if col not in df1.columns:
                    self.log_signal.emit(f"❌ 表一中缺失必要的列：{col}")
                    return

            # 对于df2，检查是否所有列都存在，如果主键是计算得到的，则不需要原始组件列
            missing_cols_df2 = []
            for col in all_needed_columns:
                if col not in df2.columns:
                    missing_cols_df2.append(col)

            # 如果有缺失的列，尝试通过计算规则创建它们
            for col in missing_cols_df2[:]:  # 使用[:]创建副本以避免在迭代时修改列表
                # 查找是否有计算规则可以生成该列
                col_rule = None
                for field1, rule in self.rules.items():
                    if field1 == col and rule.get("calc_rule"):
                        col_rule = rule
                        break

                if col_rule:
                    try:
                        calc_rule = col_rule["calc_rule"]
                        data_type = col_rule["data_type"]
                        self.log_signal.emit(f"根据规则为表二创建列：{col} = {calc_rule}")
                        df2[col] = self.calculate_field(df2, calc_rule, data_type)
                        missing_cols_df2.remove(col)  # 移除已成功创建的列
                    except Exception as e:
                        self.log_signal.emit(f"❌ 无法为表二创建列 '{col}'：{str(e)}")
                        return

            if missing_cols_df2:
                self.log_signal.emit(f"❌ 表二中缺失必要的列：{', '.join(missing_cols_df2)}")
                return
            ...
            for pk in self.primary_keys:
                df1_pk = df1[pk]
                if isinstance(df1_pk, pd.DataFrame):
                    # 如果是多列，取第一列（处理重复列名）
                    df1_pk = df1_pk.iloc[:, 0]
                df1_empty_keys = df1[pd.isna(df1_pk) | (df1_pk.astype(str).str.strip() == '')]

                # 处理表二主键空值检查
                df2_pk = df2[pk]
                if isinstance(df2_pk, pd.DataFrame):
                    # 如果是多列，取第一列（处理重复列名）
                    df2_pk = df2_pk.iloc[:, 0]
                pk_series = df2_pk.astype(str).str.strip()
                df2_empty_keys = df2[(pk_series == '') | (pk_series == 'nan')]
            ...
            if df1.index.nlevels != df2.index.nlevels:
                self.log_signal.emit(
                    f"⚠️ 警告：两个表的索引级别不一致（表一：{df1.index.nlevels}级，表二：{df2.index.nlevels}级），正在尝试修复...")

                # 统一索引级别为较高的那个
                max_levels = max(df1.index.nlevels, df2.index.nlevels)

                # 如果表一索引级别较低，添加空级别
                while df1.index.nlevels < max_levels:
                    df1.index = pd.MultiIndex.from_tuples(
                        [(idx,) + ('',) * (max_levels - df1.index.nlevels) for idx in df1.index.values],
                        names=df1.index.names + [f'level_{df1.index.nlevels}']
                    )

                # 如果表二索引级别较低，添加空级别
                while df2.index.nlevels < max_levels:
                    df2.index = pd.MultiIndex.from_tuples(
                        [(idx,) + ('',) * (max_levels - df2.index.nlevels) for idx in df2.index.values],
                        names=df2.index.names + [f'level_{df2.index.nlevels}']
                    )

            # 规范化索引格式 - 重要修复：确保索引格式一致
            # 修改规范化索引格式的部分
            def normalize_index(index):
                if index.nlevels == 1:
                    # 处理可能已经是字符串形式的元组
                    def convert_single_index(x):
                        if pd.notna(x):
                            str_x = str(x).strip()
                            # 如果是字符串形式的元组，尝试转换回元组
                            if str_x.startswith('(') and str_x.endswith(')'):
                                try:
                                    # 尝试将字符串形式的元组转换为实际元组
                                    eval_x = eval(str_x)
                    ...
                    return index.map(convert_single_index)
                else:
                    # 将多字段索引拼接为单一字符串，确保两边格式一致
                    return index.map(lambda x: ' + '.join([str(i).strip() if pd.notna(i) else '' for i in x]))

            # 应用规范化
            df1.index = normalize_index(df1.index)
            df2.index = normalize_index(df2.index)
            ...
            # 查找表二中缺失的主键 - 修复索引比较问题
            # 确保索引比较的一致性
            if df1.index.nlevels == 1 and df2.index.nlevels > 1:
                # 如果表一是单主键，表二是多主键，只取第一个主键进行比较
                df2_index_single = df2.index.map(lambda x: x[0] if isinstance(x, tuple) else x)
                missing_in_file2 = df1.index.difference(df2_index_single)
            else:
                missing_in_file2 = df1.index.difference(df2.index)
            if not missing_in_file2.empty:
                missing_df = df1.loc[missing_in_file2].copy()
                original_codes = missing_in_file2.map(lambda x: x)  # 保持原始格式
                missing_df.reset_index(drop=True, inplace=True)

                for idx, key in enumerate(self.primary_keys):
                    # 正确处理单个和多个主键的情况
                    if df1.index.nlevels == 1:
                        missing_df.insert(1 + idx, key, missing_in_file2)
                    else:
                        missing_df.insert(1 + idx, key, missing_in_file2.map(
                            lambda x: x.split(' + ')[idx] if isinstance(x, str) and ' + ' in x else
                            (x[idx] if isinstance(x, tuple) else x)))
            ...
            # 查找表二中多出的主键 - 修复索引比较问题
            missing_in_file1 = df2_index_single.difference(df1.index)
            if not missing_in_file1.empty:
                missing_df_file1 = df2.loc[missing_in_file1].copy()
                original_codes_file1 = missing_in_file1.map(lambda x: x)  # 保持原始格式
                missing_df_file1.reset_index(drop=True, inplace=True)

                for idx, key in enumerate(self.primary_keys):
                    # 正确处理单个和多个主键的情况
                    if df2.index.nlevels == 1:
                        missing_df_file1.insert(1 + idx, key, missing_in_file1)
                    else:
                        missing_df_file1.insert(1 + idx, key, missing_in_file1.map(
                            lambda x: x.split(' + ')[idx] if isinstance(x, str) and ' + ' in x else x))
            ...
            # 找出共同的主键
            common_codes = df1.index.intersection(df2_index_single)
            ...
            try:
                ...
                df1_common.index = df1_common.index.map(format_index)
                df2_common.index = df2_common.index.map(format_index)
                ...
                for field1, rule in self.rules.items():
                    ...
                    if data_type == "数值":
                        ...
                        # 日期型比较：先统一解析为标准日期格式，再按精度比较
                        def parse_date(date_str):
                            """统一解析日期字符串，支持多种格式"""
                            if pd.isna(date_str) or str(date_str).strip() == '':
                                return None
                            # 尝试解析常见日期格式（支持-和/分隔符，单/双位数）
                            try:
                        ...
                        # 判断是否都为空（空值视为一致）
                        both_empty = series1_parsed.isna() & series2_parsed.isna()

                        # 根据精度截取日期字符串（统一为YYYY-MM-DD HH:MM:SS格式后截取）
                        def format_date(date, precision):
                            if pd.isna(date):
                                return ''
                            # 转换为标准字符串格式（如2023-10-05 14:30:00）
                            date_str = date.strftime('%Y-%m-%d %H:%M:%S')
                            # 根据精度截取
                            if precision == "年":
                                return date_str[:4]  # 取年份（YYYY）
                            elif precision == "月":
                                return date_str[:7]  # 取年月（YYYY-MM）
                            elif precision == "日":
                                return date_str[:10]  # 取年月日（YYYY-MM-DD）
                            elif precision == "时":
                                return date_str[:13]  # 取年月日时（YYYY-MM-DD HH）
                            elif precision == "分":
                                return date_str[:16]  # 取年月日时分（YYYY-MM-DD HH:MM）
                            elif precision == "秒":
                                return date_str[:19]  # 取完整时间（YYYY-MM-DD HH:MM:SS）
                            return date_str

                        # 应用格式化（根据tail_diff确定精度）
                        precision = tail_diff if tail_diff in ["年", "月", "日", "时", "分", "秒"] else "日"  # 默认按日比较
                        series1_formatted = series1_parsed.apply(lambda x: format_date(x, precision))
                        series2_formatted = series2_parsed.apply(lambda x: format_date(x, precision))

                        # 比较格式化后的日期字符串
                        diff_mask = (series1_formatted != series2_formatted) & ~both_empty
            ...
            for code in common_codes:
                code_str = code if isinstance(code, str) else ' + '.join(str(c) for c in code)
                pk_mapping[code_str] = code

            for code, diffs in diff_dict.items():
                code_str = code  # code已经是字符串格式
                ...
                try:
                    ...
                    if isinstance(original_pk_values, tuple):
                        ...
                        for i, pk in enumerate(self.primary_keys):
                            condition1 = condition1 & (df1_original[pk].astype(str) == str(original_pk_values[i]))
                            condition2 = condition2 & (df2_original[pk].astype(str) == str(original_pk_values[i]))
                    else:
                        # 单主键情况
                        pk = self.primary_keys[0]
                        condition1 = (df1_original[pk].astype(str) == str(original_pk_values))
                        condition2 = (df2_original[pk].astype(str) == str(original_pk_values))
                    ...
                    if isinstance(original_pk_values, tuple):
                        for i, pk in enumerate(self.primary_keys):
                            source_dict[pk] = str(original_pk_values[i])
                            target_dict[pk] = str(original_pk_values[i])
                    else:
                        if self.primary_keys:
                            source_dict[self.primary_keys[0]] = str(original_pk_values)
                            target_dict[self.primary_keys[0]] = str(original_pk_values)
