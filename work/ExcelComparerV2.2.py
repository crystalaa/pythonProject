...
#修改为只读取规则文件的列
def read_excel_fast(file_path, sheet_name, columns_to_read=None):
    """快速读取Excel文件，支持只读取指定列"""
    try:
        wb = load_workbook(filename=file_path, read_only=True, data_only=True)
        ws = wb[sheet_name]
        ...
        if columns_to_read:
            ...
            for idx, cell in enumerate(header_row):
                ...
                if isinstance(col_name, str):
                    clean_col_name = col_name.replace('*', '').strip()
                else:
                    clean_col_name = col_name

                if clean_col_name in columns_to_read:
        ...
        for i, row in enumerate(ws.rows):
            if column_indices is not None:
                # 只读取指定列的数据
                row_data = [row[idx].value for idx in column_indices]
            else:
                # 读取所有列的数据
                row_data = [cell.value for cell in row]

            if i == 0 and columns is None:
                # 如果是表头行且尚未设置列名
                columns = row_data
            elif i > 0 or column_indices is not None:
                # 如果不是表头行，或者已经处理过表头，添加数据行
                data.append(row_data)

        wb.close()

        # 如果指定了列但没有找到任何列，返回空DataFrame
        if columns_to_read and not columns:
            return pd.DataFrame()
...
class CompareWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 获取需要读取的列（规则中定义的列）
            columns_to_read = list(self.rules.keys())

            # 同时需要读取主键列
            primary_key_columns = self.primary_keys

            # 合并需要读取的列
            all_columns_to_read = list(set(columns_to_read + primary_key_columns))
            ...
            if not missing_in_file1.empty:
                ...
                self.log_signal.emit(f"【表二中多出的主键】（表一中没有，共 {len(missing_in_file1)} 条）：\n{missing_list_file1}")
