    ...
    def run(self):
        try:
            ...
            def normalize_index(index):
                if index.nlevels == 1:
                    return index.map(lambda x: str(x).strip() if pd.notna(x) else '')
                else:
            ...
            # 查找表二中缺失的主键
            missing_in_file2 = df1.index[~df1.index.isin(df2.index)]
            ...
            # 查找表二中多出的主键
            missing_in_file1 = df2.index[~df2.index.isin(df1.index)]
